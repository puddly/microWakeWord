...
class SpectrogramGeneration:
    ...
    def get_random_spectrogram(self, mode: str | None = None):
        ...
        """
        clip = self.clips.get_random_clip(split=mode)
        if self.augmenter is not None:
            clip = self.augmenter.augment_clip(clip)

        return generate_features_for_clip(clip["audio"]["array"], self.step_ms)

    def spectrogram_generator(self, mode: str | None = None, random=False, max_clips=None, **kwargs):
        ...
        if random:
            if max_clips is not None:
                clip_generator = self.clips.random_audio_generator(split=mode, max_clips=max_clips)
            else:
                clip_generator = self.clips.random_audio_generator(split=mode)
        else:
            clip_generator = self.clips.audio_generator(split=mode, **kwargs)
